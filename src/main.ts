...
async function bootstrap() {
	const {
		LIGHTHOUSE_OPERATION_MODE,
		LIGHTHOUSE_SERVER_ID
	} = process.env;
	...
	if (LIGHTHOUSE_OPERATION_MODE === "manager") {
		...
		try {
			...
			const ServersModule = require("./modules/servers/servers.module").ServersModule;
			const ServersService = require("./modules/servers/servers.service").ServersService;
			...
			logger.error(`Failed to carry out the request ${exception}`, exception.stack);
			await app.close()
			process.exit(1)
		}
	} else {
		logger.error(`Unknown operation mode '${LIGHTHOUSE_OPERATION_MODE}' is provided.`);
	}
}
