...
export class BinaryLaneHandler extends Handler {
  ...
  constructor(provider: Provider, game: Game) {
    ...
    provider.metadata = {
      ...provider.metadata,
      ...game.data.providerOverrides.binarylane,
    };
  ...
  async createInstance(server: Server): Promise<Server> {
    ...
    try {
      ...
      while (true) {
        ...
        this.logger.debug(
          `Status: ${JSON.stringify(data.server.status, null, 2)}`,
          'waitingForServer',
        );
      ...
      const imageServer = await this.getServerIdByName(
        this.provider.metadata.binarylaneImageInstance,
      );
      const backupId = imageServer.backup_ids[0];
      const action = await this.cloneBackupToServer(
        imageServer.id,
        serverId,
        backupId,
      );
  ...
  async destroyInstance(server: Server): Promise<void> {
    ...
    // Find the server
    const targetServer = await this.getServerIdByName(
      `${server._id}.${label}.lighthouse.com`,
    );
  ...
  async cloneBackupToServer(
    serverId: number,
    targetId: number,
    backupId: number,
  ): Promise<any> {
    this.logger.debug(
      `Options: serverId(${serverId}) targetId(${targetId}) backupId(${backupId})`,
      'cloneBackupToServer',
    );
    const response = await axios.post(
      `${API_URL}/servers/${serverId}/actions`,
      {
  ...
  async waitForActionToComplete(actionId: number, retry: number): Promise<any> {
    this.logger.debug(
      `Options: actionId(${actionId}) retry(${retry})`,
